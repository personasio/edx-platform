...
class TestBulkCohorting(ModuleStoreTestCase):
    ...
    def test_no_cohort_field(self):
        ...
        response = self.call_add_users_to_cohorts('username,email\n')
        self.assertEqual(response.status_code, 400)
        result = json.loads(response.content)
        self.assertEqual(result['error'], "The file must contain a 'cohort' column containing cohort names.")
    ...
    def test_no_username_or_email_field(self):
        ...
        """
        self.client.login(username=self.staff_user.username, password='test')
        response = self.call_add_users_to_cohorts('cohort\n')
        self.assertEqual(response.status_code, 400)
        result = json.loads(response.content)
        self.assertEqual(result['error'], "The file must contain a 'username' column, an 'email' column, or both.")
    ...
    def test_wrong_extension(self):
        ...
        """
        self.client.login(username=self.staff_user.username, password='test')
        response = self.call_add_users_to_cohorts('', suffix='.notcsv')
        self.assertEqual(response.status_code, 400)
        result = json.loads(response.content)
        self.assertEqual(result['error'], "The file must end with the extension '.csv'.")
    ...
    def test_success_username(self, mock_store_uploaded_file, mock_submit_cohort_students):
        """
        ...
        self.client.login(username=self.staff_user.username, password='test')
        response = self.call_add_users_to_cohorts('username,cohort\nfoo_username,bar_cohort')
    ...
    @patch('instructor.views.api.instructor_task.api.submit_cohort_students')
    @patch('instructor.views.api.store_uploaded_file')
    def test_success_email(self, mock_store_uploaded_file, mock_submit_cohort_students):
        """
        ...
        mock_store_uploaded_file.return_value = (None, 'fake_file_name.csv')
        self.client.login(username=self.staff_user.username, password='test')
        response = self.call_add_users_to_cohorts('email,cohort\nfoo_email,bar_cohort')
        self.assertEqual(response.status_code, 204)
        self.assertTrue(mock_store_uploaded_file)
        self.assertTrue(mock_submit_cohort_students)
    ...
    @patch('instructor.views.api.instructor_task.api.submit_cohort_students')
    @patch('instructor.views.api.store_uploaded_file')
    def test_success_username_and_email(self, mock_store_uploaded_file, mock_submit_cohort_students):
        """
        Verify that we store the input CSV and call the cohorting background
